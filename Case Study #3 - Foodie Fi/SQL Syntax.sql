...
-- A. Customer Journey
SELECT s.customer_id, p.plan_id, p.plan_name, s.start_date FROM  subscriptions s 
JOIN plans p ON s.plan_id = p.plan_id;

-- B. Data Analysis Questions
-- 1. How many customers has Foodie-Fi ever had?
SELECT COUNT(DISTINCT customer_id) AS customers_Foodie_Fi
FROM subscriptions;
...
SELECT p.plan_id, p.plan_name, COUNT(s.start_date) AS year_2021 FROM  subscriptions s 
JOIN plans p ON s.plan_id = p.plan_id
...
SELECT  COUNT(*) AS churn_count, ROUND((COUNT(*)*100/ 
...
SELECT customer_id, plan_id, LEAD(plan_id, 1) OVER(PARTITION BY customer_id 
...
SELECT plan_id, COUNT(DISTINCT customer_id) AS customers FROM next_plan
